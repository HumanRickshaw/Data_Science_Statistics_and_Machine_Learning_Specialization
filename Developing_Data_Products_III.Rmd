...
The following graph shows displays the cumulative population of the US under a State of Emergency and Stay at Home mandate by their state government.  Each point can be clicked, opening a URL of the official state document (at least in most cases).
...
#Eight states and Territories have yet to issue an equivalent Stay at Home order.
not_Yet <- emergency %>% filter(is.na(emergency$Date))
...
p <- plot_ly(emergency[emergency$Type %in% c(1, 2, 2.5),],
             type = 'scatter',
             mode = 'markers',
             x = ~Date,
